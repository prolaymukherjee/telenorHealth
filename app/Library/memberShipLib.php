...
namespace App\library{
	...
	class memberShipLib
	{
		...
		public static function getNumberFormat($number){
			...
			$currencyData = Currency::findOrFail(session('configData')['currency_id']);

			if(session('configData')['currency_position']==0){
				$digitValue = $currencyData->icon.$space.$digitValue;
			}else{
				$digitValue = $digitValue.$space.$currencyData->icon;
			}
