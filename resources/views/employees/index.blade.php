@extends('layouts.layout')
@section('title', 'Employee List')
@section('content')
    <?php
    use App\Library\pmscommon;
    $add_edit = pmscommon::userWiseAccessSelection('add_edit');
    $delete = pmscommon::userWiseAccessSelection('delete');
    ?>
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1><i class="fa fa-list"></i> Employee List</h1>
        <ol class="breadcrumb">
            <li><a href="{{URL::to('dashboard')}}"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active"><a href="{{URL::to('employee-list')}}">Employee</a></li>
        </ol>
    </section>
<!-- Main content -->
<section class="content">
  <!-- Default box -->
  @include('common.message')
  <div class="box box-success">
    <div class="box-header with-border" align="right">
      @if($add_edit==true)
      <a href="{{  url('add-employee')  }}" class="btn btn-success"><i class="fa fa-plus"></i> <b>Add New</b></a> 
      @endif
      <a href="{{ url('employee-list') }}" class="btn btn-warning"><i class="fa fa-refresh"></i> <b>Refresh</b></a> 
    </div>

    <div class="box-body">
      <div class="form-group">
        <div class="row col-md-3 pull-right">
          {!! Form::open(array('url' => 'search-employee','class'=>'form-horizontal','method' =>'POST','files'=>true)) !!}
          @if(isset($autocomplete))
          <input name="user_search" value="{{$autocomplete}}" id="searchEmployee" class="form-control filter_wow" placeholder="Search by User Name . . ." type="text" selected style="border-radius: 25px;font-style: italic;">
          @else
          <input name="user_search" value="" id="searchEmployee" class="form-control filter_wow" placeholder="Search by Employee Name . . ." type="text" selected style="border-radius: 25px;font-style: italic;">
          @endif
         {!! Form::close() !!}
        </div>
        <div class="clearfix"></div>
      </div>
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-responsive">
              <th>S/L</th>
              <th>Image</th>
              <th>Employee Name</th>
              <th>Email</th>
              <th>Phone Number</th>
              <th>Designation</th>
              <th>Joining Date</th>
              <th>Action</th>
              <?php                  
                $number = 1;
                $numElementsPerPage = 10; // How many elements per page
                $pageNumber = isset($_GET['page']) ? (int)$_GET['page'] : 1;
                $currentNumber = ($pageNumber - 1) * $numElementsPerPage + $number;
              ?>
          @if(isset($searchEmployee))
          <tbody>
            @foreach($searchEmployee as $search)
              <tr>
                <td><label class="label label-success">{{$currentNumber++}}</label></td>
                <td>
                  @if($search->image !='')
                 <img src='{{asset("storage/app/public/uploads/employee/$search->image")}}' class="img-circle" style="height: 35px;width: 35px;">
                 @else
                 <img src='{{asset("storage/app/public/uploads/default.png")}}' class="img-circle" style="height: 35px;width: 35px;">
                 @endif
                </td>
                <td>{{ $search->name }}</td>
                <td>{{$search->email}}</td>
                <td>{{$search->phone_no}}</td>
                <td>{{$search->designation}}</td>
                <td>{{date('m/d/Y',strtotime($search->joining_date))}}</td>
                
                <td>
                  <div class="form-inline">

                    @if($add_edit==true)
                    <div class="input-group">
                      <a href='{{URL::to("/edit-employee/$search->id")}}' class="btn btn-primary btn-xs" style="padding: 1px 15px;">Edit</a>
                    </div>
                    @endif
                    <div class="input-group">
                      <a href='{{URL::to("/edit-employee/$search->id")}}' class="btn btn-info btn-xs" style="padding: 1px 15px;">View</a>
                    </div>
                    @if($delete==true)
                    <div class="input-group">
                      {{Form::open(array('route'=>['employee-list.destroy',$search->id],'method'=>'DELETE'))}}
                        <button type="submit" class="btn btn-danger btn-xs confirm" title="Delete" confirm="Are You Sure You Want to Delete This Employee ?" style="padding: 1px 9px;">Delete</button>
                      {!! Form::close() !!}
                    </div>
                    @endif
                  </div>
                </td>
              </tr>
              @endforeach
          </tbody>
          @else
          <tbody id="mainList">
            
            @foreach($allEmployee as $data)
              <tr>
                <td><label class="label label-success">{{$currentNumber++}}</label></td>
                <td>
                  @if($data->image !='')
                 <img src='{{asset("storage/app/public/uploads/employee/$data->image")}}' class="img-circle" style="height: 35px;width: 35px;">
                 @else
                 <img src='{{asset("storage/app/public/uploads/default.png")}}' class="img-circle" style="height: 35px;width: 35px;">
                 @endif
                </td>
                <td>{{ $data->name }}</td>
                <td>{{$data->email}}</td>
                <td>{{$data->phone_no}}</td>
                <td>{{$data->designation}}</td>
                <td>{{date('m/d/Y',strtotime($data->joining_date))}}</td>
                
                <td>
                  <div class="form-inline">
                    @if($add_edit==true)
                    <div class="input-group">
                      <a href='{{URL::to("/edit-employee/$data->id")}}' class="btn btn-primary btn-xs" style="padding: 1px 15px;">Edit</a>
                    </div>
                    @endif
                    <div class="input-group">
                      <a href='{{route("employee-list.show",$data->id)}}' class="btn btn-info btn-xs" style="padding: 1px 15px;">View</a>
                    </div>
                    @if($delete==true)
                    <div class="input-group">
                      {{Form::open(array('route'=>['employee-list.destroy',$data->id],'method'=>'DELETE'))}}
                        <button type="submit" class="btn btn-danger btn-xs confirm" title="Delete" confirm="Are You Sure You Want to Delete This Employee?" style="padding: 1px 9px;">Delete</button>
                      {!! Form::close() !!}
                    </div>
                    @endif
                  </div>
                </td>
              </tr>
            @endforeach
          </tbody>
          @endif
        </table>
        @if(isset($searchEmployee))
        <div align="center">{{ $searchEmployee->render() }}</div>
        @else
        <div align="center">{{ $allEmployee->render() }}</div>
        @endif
      </div>
    </div>

    <!-- /.box-body -->
    <div class="box-footer"> </div>
    <!-- /.box-footer-->
  </div>
</section>

<!-- /.content -->
@endsection 